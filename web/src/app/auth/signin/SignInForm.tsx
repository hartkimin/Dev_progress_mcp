...
const DEFAULT_DEV_EMAIL = 'hart@urock.kr';

export default function SignInForm({ devAuthEnabled }: { devAuthEnabled: boolean }) {
  ...
  async function handleDevLogin(e: React.FormEvent) {
    ...
    setSubmitting(true);
    await signIn('dev', { email: email.trim(), name: name.trim(), callbackUrl: '/' });
  }
    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-64px)]">
      <div className="p-8 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-md">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-8 text-center">
          Join VibePlanner to manage your projects with AI assistance.
        </p>
        ...
        {devAuthEnabled && (
          <>
            <div className="flex items-center gap-3 my-6">
              ...
              <span className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Dev Login</span>
              <div className="flex-1 h-px bg-slate-200 dark:bg-slate-700" />
            </div>
            <form onSubmit={handleDevLogin} className="flex flex-col gap-3">
              ...
              >
                {submitting ? 'Signing in…' : 'Sign in (Dev, no password)'}
              </button>
              <p className="text-xs text-slate-500 dark:text-slate-400 text-center">
                Local development only. Disabled when <code>ENABLE_DEV_AUTH</code> is not &quot;true&quot;.
              </p>
            </form>
          </>
        )}
